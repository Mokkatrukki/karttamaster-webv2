...
| `.btn--confirm` | `confirm` bg / `confirm-text` | Tallenna/Vahvista/kuittaus | `btn-bulk-apply`, `btn-bulk-checkin-aseta`, `btn-status-primary`, `btn-approve`, `btn-mark-inspected`, `btn-segment-modal-save`, `btn-segment-creation-save`, `modal-btn-primary` |
...
- **Purku-bulk (T471/V358: POISTETTU):** `.btn-bulk-collect` oli täysleveä "✓ Merkitse kaikki kerätyksi" joka näkyi purussa aina kun yksikin merkki oli kuittaamatta — eli koko työn ajan, kartan päällä. **Joukkokuittaus asuu listassa jossa on valinta:** `SegmentMarkerList`in "Valitse kaikki" + `bulkLabel`-nappi (`.btn-bulk-checkin-aseta`). Uusi joukkonappi paneeliin ⊥ palaa ilman että V358 kumotaan ensin.
