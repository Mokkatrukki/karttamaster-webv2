...
- **Lomake (`.sign-lib-form`):**
  ...
  - **Keppi-checkbox (T249/V168, `.sign-lib-keppi-checkbox`):** favorite-toggle-mallin alla, `min-height:44px`. Checked = keppi (oletus, yleisin) → näyttönimi sellaisenaan; unchecked = irto → näyttönimeen ' - irto' KAIKKIALLA (`signDisplayLabel(tpl)`, src/logic/sign-library.ts — kirjasto, picker, inventaario). Uusi malli: checked (keppi aina default ON). Inventaarion "Muuta merkiksi" (T250) avaa `keppi=true` (default) + `favorite=false`; irto-kyltit merkataan käsin poistamalla täppä. HUOM: kartan compactLabel-lyhenne johdetaan raakalabelista, EI saa suffixia.
