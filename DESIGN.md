...
- **Talkoolainen** = drive-kontrolli + oma pätkä (`RouteBar`, `src/map/route-bar.ts`): km-scrubber (`#route-track`) + ◀▶-nuolet (`#route-drive-controls`). **T224 (A)/V148:** kun talkoolaisella on pätkä, `#route-selector` näyttää reittitabien sijaan **pätkä-yhteenvedon** (`.route-segment-summary`: `📍 Oma pätkä · X.X km · N merkkiä`) — ei reittivalitsinta (35km/55km), keskitys omaan pätkään ("oma pätkä riittää"). Drive lukitaan pätkän reittiin. Reitittömälle/segmentittömälle talkoolaiselle vanhat `.route-tab`-drive-tabit säilyvät fallbackina. `#gps-drive-panel` POISTETTU (T224/F/V148) — sen ⇒ Seuraava/✓ Aseta/Ei tarpeen duplikoi SegmentView-heron; hero on ainoa seuraava-merkki-ohjaus. Oikea GPS elää `gps-navigator.ts`:ssä (ei osa route-baria).
...
- **Edistymispalkki (`.segment-view-progress`):** phase-tietoinen (`getPhaseProgress`/`formatPhaseProgress`, sama logiikka kuin järjestäjän sivupalkissa). `.segment-view-progress-bar/-fill` (`--confirm`-täyttö) + `.segment-view-progress-text` (esim "3/10 asetettu"). AINA näkyvissä otsikon alla, välilehtien ULKOPUOLELLA (myös kutistettuna).
- **Välilehdet (T224/E, `.segment-view-tabs`):** kolme `role=tablist`-nappia `[Pätkä]` · `[Kaikki merkit]` · `[Varusteet]` (`.segment-view-tab`, `44px`, aktiivi = `accent`-alaviiva; laskuri-badge `.segment-view-tab-count` merkit-/varustemäärälle). Sisältö kolmessa `.segment-view-tab-panel[data-tab]`-paneelissa, aktiivi näkyy muut `hidden`. Oletus: **Pätkä** (navigointi). Keskity yhteen asiaan kerrallaan — ei kaikkea päällekkäin.
- **Description (`.segment-view-desc`):** ohjeteksti `field-tint`-kortissa, hidden jos tyhjä. (Pätkä-paneelissa.)
- **"Seuraava merkki" -hero (`.segment-view-next`, Pätkä-paneeli, vain `phase==='asettaminen'`):** accent-vasenreunainen kortti joka ohjaa pätkän ENSIMMÄISEEN asettamattomaan merkkiin (`firstUnsetMarker`, pienin distanceFromStart). Sisältö: `buildMarkerVisual(44px)` + nimi + km + `locationNote` (ohje). **Primary 2 nappia (VISION max 2):** `✓ Aseta` (`.btn--confirm`, `flex:2`) + `Näytä kartalla` (`.btn--secondary`, panoroi + kutistaa näkymän, EI modaalia) + `⋯` (`.segment-view-next-more`, avaa overflow). **Overflow-valikko (T224/B, `.segment-view-next-menu`, hidden→toggle):** `Ei tarpeen` (`.segment-view-next-skip`, `onSkipMarker`) + `Siirretty` (`.segment-view-next-move`, `onMoveMarker`→T222; disabled jos ei annettu) + `Ota kuva`/`Laita kommentti` (`.segment-view-next-photo/-comment`, disabled "tulossa" → T221 kommentti-systeemi). "Kaikki merkit" EI valikossa (oma välilehti). Kaikki asetettu → `.segment-view-next-done` "Kaikki merkit asetettu 🎉".
- **Merkkilista (Kaikki merkit -paneeli, `.segment-view-list`):** rivit `.segment-view-item` (`buildMarkerVisual 30px` + `km — tyyppi` + `.marker-note-dot` jos locationNote + status-pilli). Klikattava (`.segment-view-item--clickable`) → `onFocusMarker` avaa `MarkerDetailModal` (ohjeet, kommentti, status). Järjestetty `distanceFromStart` mukaan (V3).
- **Varusteet (Varusteet-paneeli, `.segment-view-equipment`):** auto-laskuri tyypeittäin (ihmisluettava tyyppilabel, EI raakaa "left", readonly). **T224 (C):** manuaalirivit MUOKATTAVIA jos `onEquipmentChange` annettu (talkoolainen valmisteluvaiheessa, VISION r42/239): per rivi `count`-input + nimi-input + `✕`-poisto (`.segment-view-equipment-count/-name/-remove`, `44px`) + `+ Lisää varuste` (`.segment-view-equipment-add`). Commit `change`/blur → `updateSegment` + `PUT /api/segments/:id` (V38/V93, server sallii talkoolaisen equipment omalle pätkälle). Ilman callbackia readonly-lista.
...
- **Kokoontaitto (T223, `.segment-view--collapsed`):** chevron kutistaa näkymän otsikkoon+edistymispalkkiin → kartta saa lähes koko ruudun. "Näytä kartalla" kutsuu myös automaattisesti. Kutistettuna `.segment-view-tabs` + kaikki `.segment-view-tab-panel` `display:none` (T224).
