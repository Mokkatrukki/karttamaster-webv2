...
- **Tuleva `phase: 'tarkastus'`** (ei vielä olemassa, ks. PM-muistio): kun segmentin phase-enum laajenee 3-tilaiseksi (`asettaminen`\|`tarkastus`\|`purku`), rivi näyttäisi tarkastus-segmentille jotain "X/N tarkastettu" -tyylistä — vaatii ensin marker-tason `tarkistettu`-statuksen irrottamisen pakkogeetistä (ks. VISION-keskustelu tarkistettu-statuksen poistosta). Ei toteuteta nyt — `getPhaseProgress` kirjoitetaan niin että kolmas phase-arvo on helppo lisätä myöhemmin (switch/lookup ei if-ketju).
