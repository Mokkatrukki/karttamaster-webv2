...
- **Mobiili (≤480px):** leveän (järjestäjän) barin paneelit = **bottom sheet** (`position:fixed;bottom:0`, safe-area-padding) — kelluva paneeli veisi kartan leveyden & suodatinta säädetään juuri siksi että vaikutus nähdään kartalla (V114). Kapea (talkoolaisen) bar pitää tavallisen dropdownin: ruudun alalaidan omistaa hero (`#segment-view-container` z-index 1000). Bar `margin-left:40px` järjestäjälle — `#left-panel`in kutistettu kaista overlayaa kartan (T181/V114) & bar ⊥ saa peittää ▶-togglea.
...
### CommentThread (`.comment-thread`, `src/ui/comment-thread.ts`) — T221/T75, tyylit B105
- **Missä:** talkoolaisen SegmentView (`.segment-view-comments`, "Lisää ⋯":n jälkeen) + merkki-/pätkämodaalit. PUHDAS DOM (Vitest-jsdom).
- **Otsikko:** `Kommentit` `.marker-detail-label`. Lista: `.comment-thread-list` (tyhjä → "Ei kommentteja vielä." `text-muted 12px`).
- **Kommenttirivi (`.comment-thread-item`):** `[ikoni?] [teksti + meta] [Poista?]` — flex, `border-top border-subtle`. Ikoni `--accent`, teksti `text-primary 13px`, meta (`nimi · pvm`) `text-muted 11px`. Poisto vain `canDelete` (järjestäjä+), `modal-btn-destructive`.
- **Lomake (B105 — aiemmin 0 CSS → selaimen defaultit rikkoivat yhtenäisyyden):**
  - `.comment-thread-input` (textarea): `width:100%`, `field-tint` + `border-default` + `radius-sm`, `font-size:16px` (iOS-zoom-esto), `min-height:44px`, `resize:vertical`.
  - `.comment-thread-controls-label`: `11px text-muted` mini-label "Ikoni ja nimi (valinnainen)" — ilman tätä `Ei ikonia`-valitsin oli orpo (käyttäjä ei ymmärtänyt mihin liittyy).
  - `.comment-thread-controls`: flex, `gap:8px`, wrap. `.comment-thread-icon-select` + `.comment-thread-name`: `flex:1`, `min-height:44px`, `font-size:16px`, samat `field-tint/border-default/radius-sm`-tokenit.
...
- **Poisto:** 🗑 rivin lopussa, `modal-btn-destructive`-väri, vahvistus ennen `DELETE`. Vain järjestäjä+ (backend V13).
- **Touch:** rivi ≥44px korkea myös yhdellä tekstirivillä (järjestäjä on desktopilla, mutta sama sopimus pitää rivin luettavana).
