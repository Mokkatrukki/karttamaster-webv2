...
- **Lomake (`.sign-lib-form`):**
  ...
  - ~~**Keppi-checkbox (T249/V168, `.sign-lib-keppi-checkbox`)**~~ **POISTETTU T255/V177** — kiinnitystapa (keppi/irto) EI ole enää mallin kenttä vaan **inventaariorivin** attribuutti (sama kylttipinta yhdellä tunnuksella voi olla kepillä JA irtona). Malli = pelkkä kylttipinta. Kiinnitys valitaan inventaarion Tiedot-editorissa: **"Keppi (pois = irto)" -täppä (`.inv-d-keppi`)** VAIN merkkirivillä (`item.templateId`), `.inv-field-keppi`-kuoressa. Pois = irto → `' - irto'` -suffix VAIN inventaarion rivinäytössä (`resolveItemName` → `signDisplayLabel(tpl.label, item.keppi)`); kirjasto/picker/kartta = pelkkä `tpl.label`. HUOM: kartan compactLabel-lyhenne johdetaan raakalabelista, EI saa suffixia.
