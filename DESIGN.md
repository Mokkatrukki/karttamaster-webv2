...
- **Kokoontaitto (T223, `.segment-view--collapsed`):** chevron kutistaa näkymän otsikkoon+edistymispalkkiin → kartta saa lähes koko ruudun. "Näytä kartalla" kutsuu myös automaattisesti. Kutistettuna desc/next/list/varuste-btn/bounds/inspect/bulk `display:none`.
...
- **Seuraava merkki kartalla (T224/b1, `NextMarkerHighlight` `src/map/next-marker-highlight.ts`):** pätkän seuraava asettamaton merkki korostuu kartalla accent-renkaalla (`.next-marker-ring`, hienovarainen pulssi, ei-interaktiivinen) — talkoolainen näkee heti minne mennä. Kartta on päänavigointipinta (alapalkki poistettu). Vain `asettaminen`-phase.
