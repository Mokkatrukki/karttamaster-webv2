...
- **Read/edit-moodi (T251, `viewMode` 'read'|'edit', V169–V171):** sivu avautuu **read-modessa** (oletus, sessiokohtainen — reload→read, V170). **Yläpalkki `.inv-topbar`** ensimmäisenä: **moodi-toggle `.inv-mode-toggle`** (pilli `border-radius:999px`, `min-height:44px`, `font-size:14px`; teksti read="✎ Muokkaa" / edit="✓ Valmis"; `.active`=accent-täyttö kuten `.inv-loc-tab.active`, `aria-pressed`) + edit-modessa `.inv-mode-badge` "Muokkaustila" (`accent`-teksti). **Edit-signaali = 3 kerrosta:** (1) toggle-teksti+accent-täyttö, (2) mutaationappien ilmestyminen, (3) container-tason **`.inv-edit-mode { box-shadow: inset 3px 0 0 accent }`** — inset vasen väripalkki (`marker-item--pending`-idiomi, EI border/padding → ei reflowta togglatessa; persistoi koko listan yli perifeerisenä vihjeenä). **Read piilottaa KAIKKI mutaatiot** (add-form, `.inv-stepper`, `.inv-card-actions`, `.inv-loc-add`, `.inv-loc-edit-toggle`); **säilyttää katselun**: paikkatabit, "Kaikki", zoom-lightbox, kyltin nimi→SignTemplateModal (tietoinen editori sallittu read-modessa). **Read-rivi `.inv-card--read`** = tiivis yksirivi (`padding:8px 12px`, määrä pelkkänä tekstinä `.inv-card-qty` `margin-left:auto` → `[ikoni] nimi ··· määrä unit`). **"Kaikki"+edit:** rivi-muokkaus ON, add-form EI (V171, add vaatii paikkakontekstin). Rooli: järjestäjä/admin (mobiili-primääri kärryllä). Ei uusia väritokeneja (reuse accent/field-tint).
