...
// T224 (A): talkoolaisen alapalkki ei näytä reittivalitsinta (35km/55km) vaan OMAN pätkän
// yhteenvedon — "keskity omaan pätkään", VISION "oma pätkä riittää". Reittiajo-koneisto (scrubber,
// polyline-klikit, drive-reitti) säilyy; vain tab-UI korvataan yhteenvedolla + drive lukitaan pätkän reittiin.
export interface RouteBarSegmentSummary {
  routeId: string       // pätkän reitti johon drive/scrubber lukitaan
  lengthKm: string      // esim "7.0"
  markerCount: number
}
...
export class RouteBar {
  ...
  constructor(
    ...
    // Jos annettu: renderöi pätkä-yhteenveto tabien sijaan (talkoolainen).
    segmentSummary?: RouteBarSegmentSummary,
  ) {
    this.visibleRouteIds = routes.map(r => r.id)
    // Lukitse drive pätkän reittiin jos annettu ja se on olemassa; muuten ensimmäinen reitti.
    this.driveRouteId = segmentSummary && routes.some(r => r.id === segmentSummary.routeId)
      ? segmentSummary.routeId
      : routes[0].id
    if (segmentSummary) this.buildSegmentSummary(segmentSummary)
    else this.buildTabs()
  ...
  // T224 (A): pätkä-yhteenveto tab-palkin tilalle — pituus + merkkimäärä, ei reittivalitsinta.
  private buildSegmentSummary(s: RouteBarSegmentSummary): void {
    const wrap = document.createElement('div')
    wrap.className = 'route-segment-summary'

    const icon = document.createElement('span')
    icon.className = 'rss-icon'
    ...
    title.textContent = 'Oma pätkä'
    wrap.appendChild(title)

    const meta = document.createElement('span')
    meta.className = 'rss-meta'
    const merkkiWord = s.markerCount === 1 ? 'merkki' : 'merkkiä'
    meta.textContent = `${s.lengthKm} km · ${s.markerCount} ${merkkiWord}`
