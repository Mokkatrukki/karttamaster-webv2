...
function circleSvg(type: MarkerType, bearing: number, status: MarkerStatus): string {
  ...
  const dotHtml = dotColor
    ? `<span style="position:absolute;bottom:2px;right:0px;width:8px;height:8px;border-radius:50%;background:${dotColor};border:1.5px solid #fff;pointer-events:none"></span>`
    : ''
