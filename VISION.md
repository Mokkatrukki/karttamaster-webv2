...
8. **Pysäköintipiste / "mihin auto jätetään" (avattu 2026-07-29):** merkit ovat metsässä — talkoolainen ajaa mahdollisimman lähelle ja kävelee loput. Kysymys: mistä nav-linkin (ks. §Kenttätyö "Navigoi tähän") kohde tulee? **Ei lasketa tieverkosta:** routing-palvelu vaatii verkkoyhteyden juuri siellä missä sitä ei ole, ja lopputulos on silti arvaus (nav-appi snappaa metsämerkin usein väärälle puolelle → kiertotie). Ratkaisu on ihmistieto: järjestäjä tuntee reitin ja tietää mihin auto jätetään → **pysäköintipiste-POI** jonka järjestäjä asettaa kartalle. Nav-linkki osoittaa siihen jos pätkällä sellainen on, muuten merkkiin itseensä. Sukua §Erikoismerkit-tarinan noutopisteelle/pudotuspisteelle. **Riippuu kohdasta 7** (POI-tietomalli) — ei speksata ennen sitä. Väliaikaisratkaisu käytössä siihen asti: linkki merkin omiin koordinaatteihin.
