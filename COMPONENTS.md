...
| SegmentManager | `src/logic/segments.ts` | ✓ T465 | — (Taso-1: `tests/segments.test.ts`) | [logic.md](docs/components/logic.md) |
| SegmentColor | `src/logic/segment-color.ts` | ✓ T465 | t464-segment-boundaries: naapuriväritys kartalla (V352) | [logic.md](docs/components/logic.md) |
...
| PhaseView | `src/logic/phase-view.ts` | ✓ T148, T426/V317 (aktiivinen vaihe = serverin tila, localStorage vain välimuisti), T434/V321 (KAKSI käsitettä: globaali `getActivePhase` = adminin komento · `getViewPhase`/`setViewPhase`/`isViewingOtherPhase` = järjestäjän katselu, paikallinen ⊥ serverikutsua) | tests/t426-phase-state.test.ts, tests/t434-view-phase.test.ts | [logic.md](docs/components/logic.md) |
| PhaseLabels | `src/logic/phase-labels.ts` | ✓ T433/T435 (vaiheen nimet + vaiheenvaihdon SEURAUSLAUSE + HTTP-virhe→toimenpide -kartta; oma moduuli ∴ ⊥ peri phase-view'n localStorage-sivuvaikutusta) | tests/t433-admin-phase.test.ts | [logic.md](docs/components/logic.md) |
...
| NavigationLogic | `src/logic/navigation.ts` | ✓ T16, T327, T328 (pätkäkontekstin funktiot saavat `segment`in ⊥ `routeId`-parametria, V237; drive-funktiot pitävät `routeId`:n — eri omistaja), T413/V304 (`nearestUnsetByGps` + `defaultUnsetSelection`: reitittömän tehtävän oletusvalinta metrisesti GPS-fixistä, lista & valittu merkki pysyvät vakaina), T459/V344 (navigoitava joukko = kyltit; `markerTypeFilter`-tehtävä määrittelee joukkonsa itse — kasa ⊥ nouse heroon, B191) | tests/navigation.test.ts; tests/t413-gps-nearest.test.ts | [logic.md](docs/components/logic.md) |
...
| SegmentOverlay | `src/map/segment-overlay.ts` | ✓ T466 | t464-segment-boundaries: päätepistemerkit + klikki casingille (V353/V345); t419-label-scale; t349-map-surface-theme; critical-paths: nimilapun klikkaus + V142-dim; segments: viivatyyli koodaa statuksen (V252) | [map.md](docs/components/map.md) |
| SegmentEditHandles | `src/map/segment-edit-handles.ts` | ✓ T466 | segments: luonnin snap-flow + rajakahvat | [map.md](docs/components/map.md) |
...
| PlaceMode | `src/ui/place-mode.ts` | ✓ T172, T454 (V339: viritys purkautuu napautuksessa, TILAN siivous vasta kun tulos on käytetty) | critical-paths: "Merkki kartalle", "dblclick", "sivupalkin merkkikirjastosta" | [ui.md](docs/components/ui.md) |
...
| MapClickOwner | `src/logic/map-click.ts` | ✓ T460/V345 (kuka omistaa kartan napautuksen: virittäjä varaa, kerrokset luovuttavat yhdellä rivillä — ⊥ viittä `if (armed)`-haaraa, B192) | tests/t460-map-click.test.ts; tests/t460-place-mode-claim.test.ts; e2e/t452-pile-place.spec.ts (T460) | [logic.md](docs/components/logic.md) |
| PileEditWindow | `src/app/pile-edit-window.ts` | ✓ T457/V342 (juuri jätetty kasa on raahattava kunnes käyttäjä tekee seuraavan teon — ⊥ ajastinta) | tests/t457-pile-edit-window.test.ts; e2e/t452-pile-place.spec.ts (T457) | [ui.md](docs/components/ui.md) |
| RouteTrace | `src/map/route-trace.ts` | ✓ T458/V343 (reittijäljet `/kasat`-kartan taustaksi; lataus ⊥ blokkaa listaa, GPX-virhe ⊥ näy) | e2e/t458-kasat-routes.spec.ts | [ui.md](docs/components/ui.md) |
...
| SegmentView | `src/ui/segment-view.ts` | ✓ T228, T218 (dynaaminen keräyslista), T234 (562r koordinaattori; hero irrotettu), T471/V358 (paneelin joukkonappi poistettu — joukkokuittaus asuu merkkilistassa) | tests/t14-segment-view.test.ts, tests/t224-segment-view.test.ts, tests/t218-collection-list.test.ts; e2e/segments.spec.ts | [ui.md](docs/components/ui.md) |
| SegmentHero | `src/ui/segment-hero.ts` | ✓ T429/V319 (purussa tasan 2 toimintoa), T422/V313 (purku-hero: sama hero palvelee asettamista JA purkua, sanat `phaseTarget`-lookupista), T234 (seuraava-merkki-hero + ◀▶-nav + selectedNavId V159, irrotettu SegmentViewsta), T327 (järjestys pätkän primary-reitin km:llä, V235), T413/V304 (oletusvalinta `defaultUnsetSelection`illa: reitittömällä tehtävällä lähin GPS-fixiin; hysteresis tulee V159-reconcilesta) | tests/t232-segment-view-hero.test.ts; tests/t413-hero-gps-selection.test.ts | [ui.md](docs/components/ui.md) |
...
| PhaseSwitcher | `src/ui/phase-switcher.ts` | ✓ T148,T180 (stopPropagation, B80), **T434/V321: KATSELUSUODIN ⊥ komento** — ⊥ kirjoita serverille, pilleri näyttää eron kun katselu ≠ käynnissä oleva vaihe | e2e/t180-phase-switcher-menu.spec.ts, tests/phase-switcher.test.ts, tests/t434-view-phase.test.ts | [ui.md](docs/components/ui.md) |
...
| AdminPage | `admin.html` + `src/admin.ts` + `src/ui/admin-page.ts` | ✓ T122, T268 (renderAdminSettings: talkoo-salasana), T270 (FAQ), **T433/V321 `renderAdminPhase`: tapahtuman vaiheen vaihto — nykytila ensin + seurauksen sanova vahvistus + eritelty virhe (V322); ainoa kirjoituspolku `PUT /api/phase`:een (admin-only T432)** | tests/t433-admin-phase.test.ts | [ui.md](docs/components/ui.md) |
| PatkatPage | `patkat.html` + `src/patkat.ts` + `src/ui/patkat-page.ts` | ✓ T271 (talkoolais-hub: hero+FAQ+pätkälista+Kartalle; sanitizeHtml V190; auth-gate), T470 (vaihesuodin ∀ roolille + järjestäjän PhaseSwitcher, V357) | e2e: — (jsdom t271; Playwright deferred) | [ui.md](docs/components/ui.md) |
...
| `src/logic/segments.ts` | ✓ T465 | PILKOTTU 2026-08-02: värikieli → `segment-color.ts`. Jakolinja väri/status — `segmentLineState` jäi koska se lukee `PhaseProgress`ia (kehä muuten) |
| `src/map/segment-overlay.ts` | ✓ T466 | PILKOTTU 2026-08-02: rajakahvat + luonnin snap-merkit → `segment-edit-handles.ts`. Jakolinja seurasi TILAA: kumpikaan ⊥ koske `layers`-listaan ∴ V353(e):n `clear()`-sopimus ⊥ ristikkäisty |
