...
| SegmentVisibility | `src/logic/segment-visibility.ts` | ✓ T374 (`segmentVisibleOnRoutes`: reittinäkyvyys koskee myös pätkäviivoja & nimilappuja, jäsenyys primarysta V211, reititön ⊥ katoa V139; V269/B157), ✓ T418 (`segmentLabelVisible` + `MIN_SEGMENT_LABEL_ZOOM` 14: nimilappu on zoom-ehdollista sisältöä, oma pätkä ohittaa; V309) | tests/t374-segment-visible-routes.test.ts; tests/t418-segment-label-zoom.test.ts; e2e/segments.spec.ts | [logic.md](docs/components/logic.md) |
...
| SegmentOverlay | `src/map/segment-overlay.ts` | ✓ T152, T217 (reititön skip), T347 (nimilappu klikattava), T348 (valmis = vihreä + ✓-lappu), T418 (nimilappu piiloon zoom < 14, `zoomend` + render-loppu; V309/V310/V311) | segments: "viivatyyli koodaa statuksen" (dashArray-ARVO, V252) + "valmis pätkä = vihreä ehjä viiva + ✓-nimilappu (T348)"; t217: routeless gap-safety (Taso-1, Playwright post-T216); t347: nimilapun klikkaus + drag-panorointi + V142-dim (`critical-paths.spec.ts`); t349: karttapinnan teemariippumattomuus — sama näkymä light+dark, värit identtiset (`t349-map-surface-theme.spec.ts`, V253); t336: casing 3 kerrosta (Taso-1 `t336-segment-casing.test.ts`) | [map.md](docs/components/map.md) |
