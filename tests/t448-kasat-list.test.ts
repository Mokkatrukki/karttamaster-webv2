...
import { listPiles, formatPileDistance, formatPileSummary, PILE_TARGET } from '../src/logic/pile-list'
...
describe('T448/V332 — kasalista johdetaan kasoista, ⊥ tehtäväoliosta', () => {
  it('lista on `kind === kasa` -suodatin koko merkkijoukosta', () => {
    const rows = listPiles([marker('kyltti1'), pile('k1', 65.6, 27.5), marker('kyltti2')])
  ...
  it('tyhjä joukko → tyhjä lista (⊥ kaadu, ⊥ arvaa)', () => {
    expect(listPiles([])).toEqual([])
    expect(listPiles([marker('a')])).toEqual([])
  })
  ...
  it('järjestys: lähin ensin kun GPS-fix on', () => {
    ...
    const rows = listPiles([kaukana, keski, lahella], { lat: 65.6, lon: 27.5 })
  ...
  it('ilman fixiä järjestys on luontijärjestys & etäisyys on null (⊥ arvattu nolla)', () => {
    const rows = listPiles([pile('b', 65.9, 27.5), pile('a', 65.61, 27.5)])
    expect(rows.map(r => r.marker.id)).toEqual(['b', 'a'])
    expect(rows.every(r => r.distanceM === null)).toBe(true)
  })

  it('haettu kasa ⊥ katoa listalta mutta putoaa hännille', () => {
    const rows = listPiles(
      [pile('haettu', 65.601, 27.5, { status: 'kerätty' }), pile('avoin', 65.8, 27.5)],
      { lat: 65.6, lon: 27.5 },
    )
    // Haettu on FYYSISESTI lähempänä mutta ⊥ ole työtä ∴ avoin on ensin.
    expect(rows.map(r => r.marker.id)).toEqual(['avoin', 'haettu'])
    expect(rows.find(r => r.marker.id === 'haettu')!.done).toBe(true)
  })

  it('sisältömäärä tulee pileMarkerIds:istä (puuttuva → 0, ⊥ undefined)', () => {
    const rows = listPiles([pile('k', 65.6, 27.5, { pileMarkerIds: ['a', 'b', 'c'] }), pile('tyhja', 65.6, 27.5, { pileMarkerIds: undefined })])
