...
describe('MarkerDetailModal', () => {
  ...
  it('valinnan muutos kutsuu manager.updateType uudella tyypillä', () => {
    ...
    expect(manager.updateType).toHaveBeenCalledWith('test-id', 'left', undefined, undefined, undefined, undefined)
    expect(onUpdate).toHaveBeenCalled()
  })
  ...
  it('kirjaston templaatin valinta välittää color+label updateType:lle', () => {
    ...
    select.dispatchEvent(new Event('change'))
    expect(manager.updateType).toHaveBeenCalledWith('test-id', 'custom-1', '#ff0000', 'Vaara', undefined, undefined)
  })
})
