...
describe('T336/V244: casing-kerrokset', () => {
  it('reitillinen + korostettu pätkä → kolme kerrosta piirtojärjestyksessä', () => {
    const layers = segmentLayerStyles({ segmentColor: '#552070', routeColor: '#1D8CB4', base: BASE, highlighted: true, interactive: true })
  ...
  it('statuskieli elää CASINGISSA — sisus pysyy ehjänä', () => {
    const layers = segmentLayerStyles({ segmentColor: '#552070', routeColor: '#1D8CB4', base: DASHED, highlighted: true, interactive: true })
  ...
  it('reititön tehtävä (V139) → yksi viiva, ⊥ kaadu puuttuvaan reittiväriin', () => {
    const layers = segmentLayerStyles({ segmentColor: '#552070', routeColor: undefined, base: BASE, highlighted: true, interactive: true })
  ...
  it('V142: himmennetty konteksti-pätkä → yksi viiva, ⊥ casingia (korostuksen kieltä ⊥ anneta taustalle)', () => {
    ...
    const layers = segmentLayerStyles({ segmentColor: '#552070', routeColor: '#1D8CB4', base: dim, highlighted: false, interactive: false })
    expect(layers).toHaveLength(1)
